...
本文涉及的 4 种分布式事务解法（RocketMQ 事务消息、本地消息表、TCC、Saga）均来自社区公开文档与架构实践。事故案例为社区公开经验总结，已脱敏处理。跨业务形态 SOP 对比、跨系统对比均为社区共识总结。
