...
## 3. 主流业界解法：3 种业务形态的事务消息实践

### 3.1 保守型（金融 / 资金场景）

**RocketMQ 孵化方（保守型）是最早的生产实践**，事务消息从内部 Notify 系统开始就一直在用。生产 SOP：
...
高吞吐型配置照搬到资金场景 → 刷盘策略用 ASYNC_FLUSH → 资金消息丢失。
...
本文涉及的 RocketMQ 事务消息机制、半消息原理、回查策略、16 级重试阶梯均来自 RocketMQ 4.x / 5.x 官方文档与社区公开 issue。事故案例为社区公开经验总结，已脱敏处理。3 种解法对比、跨业务形态 SOP 对比、跨系统对比均为社区共识总结，非单一来源。
...
| 跨业务形态配置照搬 | ASYNC_FLUSH 用于资金 | 按业务分级 |
