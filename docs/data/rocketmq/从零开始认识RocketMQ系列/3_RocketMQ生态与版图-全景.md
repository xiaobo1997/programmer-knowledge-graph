...
跨公司视角：阿里、字节、美团、京东、拼多多都在用 RocketMQ（或改造版）；海外 Apache RocketMQ 用户覆盖东南亚、欧洲部分企业；这是「**国内主导 + 海外逐步渗透**」的生态特征。
...
跨公司视角：阿里系业务全面切到 5.x（Controller 模式）；字节、美团大量自研 MQ 兼容 Kafka 协议；传统金融仍用 4.x。这不是「谁对谁错」，而是「**业务体量决定演进节奏**」。
