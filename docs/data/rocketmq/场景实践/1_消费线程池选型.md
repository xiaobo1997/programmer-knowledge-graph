...
| 指标 | 预警 | 告警 | 含义 |
|---|---|---|---|
| consumeRT | >100ms | >500ms | 消费耗时 |
| consumeFailedMsgs | >5/min | >20/min | 失败消息 |
| reconsumeTimes | >3 | >16 | 重试次数 |
| pullRT | >50ms | >200ms | 拉取耗时 |
| pullTPS | 突变 | 跌零 | 拉取吞吐 |
| consumeTPS | 突变 | 跌零 | 消费吞吐 |
| queueOffsetDiff | >1000 | >10000 | 堆积量 |

### B.3 3 大贯穿维度对照表

| 场景 | 设计思想 | 设计方案 | 取舍依据 |
|---|---|---|---|
| 支付回调 | 强一致优先 | 方案 A（顺序消费单队列） | 回调乱序会导致重复入账，顺序消费换吞吐，值得 |
| 日志收集 | 高吞吐优先 | 方案 B（并发消费批量） | 日志允许乱序，并发 + 批量换 10 倍吞吐 |
| 数据同步 | 业务连续性 | 方案 C（协程消费池化） | 重活慢，给足超时防重投风暴 |
| 风控计算 | 故障优先 | 方案 B（并发消费批量） | 计算可重算，失败了重跑就行，不需要顺序 |

**这就是"3 大贯穿维度"在本实践中的落地**——不是塞在某节，而是每节都要回答"为什么这样做 / 怎么做 / 权衡了什么"。不同业务场景取舍不同，没有标准答案，按自己的场景定。
