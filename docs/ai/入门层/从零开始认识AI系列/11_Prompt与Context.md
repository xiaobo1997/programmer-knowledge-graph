...
**本文核心**：PE 是"把一句话问好"，CE 是"把模型看到的整个世界设计好"——Agent 时代竞争力在后者：同一模型，上下文组装不同，表现天差地别。上下文工程四支柱（Write/Select/Compress/Isolate）是每个 AI 应用工程师必须掌握的工程纪律。

▶ 快速定位：PE vs CE（§1）· CoT（§2）· 系统提示词（§3）· 上下文退化（§4）· 结构化输出（§5）· 提示注入（§6）
...
**追问**：多 Agent 返回矛盾时怎么裁决？→ 统一事实源 + 冲突显式裁决（优先级：系统提示 > 检索 > 历史）。
...
一句话复述：上下文工程四支柱（Write 持久化 / Select 检索 / Compress 压缩 / Isolate 隔离）是 Agent 时代的工程底线——长上下文不是资产是负债，超过某个规模后"更多历史"变成"更差判断"。

## tips

- 反直觉点：上下文组装是系统设计问题，不是写代码时的随手拼——定模板（指令/资料/问题三段式）+ 版本化 + 进评测
- 防御话术："我们确实遇到过上下文退化。解法是分三层：关键事实外置存储 + 阶段性重开上下文 + 工具返回裁剪——这三层缺一不可，只做一层防不住"
- 考场应急：被问"为什么不能直接加长窗口"→ "窗口是装得下，注意力是用得好——U 形注意力下中间段约等于没放，堆窗口不解决退化，必须主动管理"

## QA（你们可能会问）

| 问题 | 答案 |
|------|------|
| 有什么数据证明上下文比模型重要？ | Chroma 的 context rot 实验：GPT-4o 同一信息换个结构，准确率掉 34 个百分点——差距在管理不在模型 |
| 子 Agent 隔离值得吗？ | Anthropic 实测：并行子 Agent token 成本 ×15，换 90% 任务质量净收益——隔离是用钱买质量和并行度，按任务价值决定 |
| 压缩丢关键信息怎么办？ | 分层压缩：结构化事实（单号/金额/决策）原样保留，叙述性历史转摘要——摘要模型可以微调专做"精确信息保留" |

## 思考

- 上下文退化能否提前预测？→ 目前靠 Eval 分桶检测，是否有在线信号（如工具调用重复率）可早期预警？
- 子 Agent 隔离的边界在哪？→ 15× token 成本换 90% 净收益，但简单任务不值得隔离——"任务价值 vs 隔离成本"二维决策