...
不同量级下，接入层的形态完全不同：10 万 QPS 以内一套 Nginx 加托管 WAF 就够；千万级日活要上智能 DNS 与多机房接入；亿级流量必须把静态内容全部下沉 CDN、入口按单元就近接入。同一个「接入层」三个词，三种复杂度——量级没到就上全套，是在为想象中的流量买单。

值得追问的一个反问：接入层加了这么多道关卡，哪一道最常出问题？答案是「最近一次改配置的那道」。一次真实的接入层事故：某团队为防爬虫收紧了 WAF 规则，正常用户的支付回调被当成爬虫拦了，支付状态不同步，客诉涌进来——拦截规则变了，但没人验证过「合法流量」长什么样。教训写成一句话：**接入层每一道防护的变更，都要先回答「它会不会误伤正常流量」，用真实流量形态回放验证，而不是只测「挡不挡得住攻击」**。
...
再补一个接入层的量级常识：**CDN 的命中率是「设计」出来的不是「买」出来的**。同样的 CDN 服务，缓存键设计得合理（剔除无关参数）、缓存过期时间分层设置、静态资源带内容哈希的团队，命中率能做到九成以上；随手接入的团队可能只有六七成——差距全部落在接入成本与回源带宽的账单上。接入层的每一项配置都值得用「流量形态」校准一遍：你的用户在哪里（决定节点选择）、你的内容什么比例是静态（决定下沉策略）、你的流量什么时间到峰（决定弹性策略）——三个问题答清楚，接入层的选型与参数就有了骨架。
