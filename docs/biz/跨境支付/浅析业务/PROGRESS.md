...
- [1 · 参与方全景与利益博弈-深度](./1_参与方全景与利益博弈-深度.md) — 9 类参与方 + 议价权 + 风险归属（**已完成**）

### 第二层 · 交易链路与时点（穿透每一跳）

- [2 · 外卡支付链路与 13 时点-深度](./2_外卡支付链路与13时点-深度.md) — 授权/捕获/清算/结算 4 阶段 + 13 时点 + chargeback（**已完成**）
- [3 · 业务模式与费率分润-深度](./3_业务模式与费率分润-深度.md) — 聚合 vs 直连 vs 本地 + 5 项费率拆解 + 分账（**已完成**）

### 第三层 · 架构与系统（怎么落地）

- [4 · 收单系统架构全景-全景](./4_收单系统架构全景-全景.md) — 6 层功能架构 + 部署拓扑 + 技术栈（**已完成**）
- [5 · 通道管理与路由策略-深度](./5_通道管理与路由策略-深度.md) — 4 类通道 + 5 大属性 + 路由 + 生命周期（**已完成**）
- [6 · 跨境对账与结算体系-深度](./6_跨境对账与结算体系-深度.md) — 三方对账 + 结算周期 + 换汇 3 决策（**已完成**）
- [7 · 多币种账户体系-深度](./7_多币种账户体系-深度.md) — 账户模型 + 状态机 + 换汇策略（**已完成**）
- [8 · 风控与合规框架-深度](./8_风控与合规框架-深度.md) — 风控 3 层 + 3DS v1/v2 + AML + 国际合规 8 标准（**已完成**）

### 第四层 · 业务能力矩阵（能力对照）

- [9 · 端到端实战-境外卡全链路-业务深度](./9_端到端实战-境外卡全链路-业务深度.md) — 1 真实场景走通 8 步全链路（**已完成**）
- [10 · 全局架构设计与决策清单-全景](./10_全局架构设计与决策清单-全景.md) — 4+1 视图 + 16 项技术栈 + 8 大 trade-off（**已完成**）
...
- [13 · 核心功能 12 场景-深度](./13_核心功能用例与12场景-深度.md) — 12 个标准化功能 + 用例模板（**已完成**）

### 第五层 · 业务形态分类（按形态展开）

- [14 · 线上支付全流程-PC 与 H5-深度](./14_线上支付全流程-PC收银台与H5-深度.md) — 3 种线上支付形态 + PC + APP 内（**已完成**）
- [15 · POS 刷卡支付与 EMV-深度](./15_POS刷卡支付-终端与卡组织-深度.md) — 线下 CP 模式 + EMV 芯片 + 3 种读卡（**已完成**）
...
| 🔴 高 | 1 | 参与方全景与利益博弈 | 9 类参与方 + 议价权 + 风险归属 | ✅ 已完成 |
| 🔴 高 | 2 | 外卡支付链路与 13 时点 | 4 阶段 + 13 时点 + chargeback | ✅ 已完成 |
| 🔴 高 | 3 | 业务模式与费率分润 | 聚合 vs 直连 + 5 项费率 | ✅ 已完成 |
| 🟡 中 | 4-8 | 架构与系统（5 篇）| 6 层架构 + 通道 + 对账 + 账户 + 风控 | ✅ 已完成 |
| 🟡 中 | 9 | 端到端实战-境外卡全链路 | 8 步全链路 + 6 机构 | ✅ 已完成 |
| 🟡 中 | 10-13 | 能力矩阵（4 篇）| 全局架构 + 支付方式 + 订单 + 核心功能 | ✅ 已完成 |
| 🟢 低 | 14-17 | 业务形态（4 篇）| 线上 + POS + 代付 + 多币种 | ✅ 已完成 |
| 🟢 低 | 18 | 跨境支付进化论与未来展望 | 5 关键节点 + 4 维协同 + 5 项核心能力 + 4 浪叠加 | ✅ 已完成 |
