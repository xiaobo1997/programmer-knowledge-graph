...
| 量级 | Lambda 与函数式接口的形态变化 | 关注点 |
|---|---|---|
| 10 万级 | 入门阶段，Lambda 接收默认配置即可 | 语法正确性 |
| 千万级 | 需要关注 函数组合 的取舍 | 性能瓶颈与参数调优 |
| 亿级 | Lambda 接收 成为架构决策的核心变量 | 架构选型与底层机制 |

量级每上一档，对 Lambda 接收 的容忍度就降一级——**同样的代码在十万级没问题，在亿级就是事故预备役**。
