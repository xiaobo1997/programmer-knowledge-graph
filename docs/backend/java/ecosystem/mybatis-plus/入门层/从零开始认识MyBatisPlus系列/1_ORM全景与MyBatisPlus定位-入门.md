...
```mermaid
flowchart LR
    A["JDBC<br/>全手写 7 步样板"] -->|痛点: 样板太多| B["JPA/Hibernate<br/>全自动 放弃 SQL 控制"]
    A -->|痛点: SQL 不可控| C["MyBatis<br/>半自动 SQL 归你 样板归它"]
    C -->|痛点: 单表 CRUD 仍重复| D["MyBatis-Plus<br/>只增强不改变 收走单表样板"]
    B -.->|国内主流倒向| C
...
```mermaid
flowchart TB
    BIZ["业务代码"] --> MP["MyBatis-Plus<br/>BaseMapper 通用 CRUD + 条件构造器 + 插件"]
    BIZ --> MB["MyBatis 原生<br/>XML / 注解自定义 SQL - 复杂查询主战场"]
    MP --> MB
    MB --> JDBC["JDBC"]
    JDBC --> DB[("数据库")]
...
## 4. 边界辨析：MP 不做什么（常见误区与不适用场景）

理解一个工具，更要理解它的边界。MP 的「只增强」集中在**单表操作**，它明确不越界的地方有几类：

从分层架构看，MP 处在持久层的**上游包装位**：它依赖 MyBatis 的执行体系（SqlSession/Executor/StatementHandler 四大对象），自身不做连接管理和 SQL 执行——所有请求最终都下沉到 MyBatis → JDBC 这条链路。理解这个上下游关系，就知道 MP 的问题排查最终都会落到 MyBatis 层。
...
1. **MyBatis-Plus 会取代 MyBatis 吗？**
   - 不会，也没打算。MP 是 MyBatis 的增强层，复杂 SQL 你照样写 XML——两者是同一套体系里的分工，不是新旧替代。

2. **MP 和 JPA 该怎么选？**
   - 看团队跟 SQL 的关系：SQL 要审、要优化、要复盘（金融/交易类）选 MyBatis 系；领域模型复杂、以标准 CRUD 为主可考虑 JPA。国内主流是 MyBatis 系。

3. **用了 MP 就不用写 SQL 了吗？**
   - 单表 CRUD 确实零 SQL，但多表 join、复杂统计照写 SQL——MP 的边界就在单表，越界部分与 MyBatis 原生能力共存。
