...
入门层 0-23 已完成（Boot 3.5 主线）；**特性层 4 子系列（8 篇）与专题层 3 子系列（9 篇）已全量落盘**——主线 **Spring Boot 4.1.1 / Spring Framework 7.0.9**（2026-09-10 gh 实测），3.x 差异标注，全部过 gate 95-100 分。整合层按规划「主题待定」（等真实业务实践落位）。各子系列篇目见其 index 的「已落盘篇目」节。
