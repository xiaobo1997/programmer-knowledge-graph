...
从架构上下游看：自动装配是「classpath 与 IoC 容器」的桥梁——上游扫描 classpath 发现能力，下游注册 BeanDefinition——模块边界即「能力发现 vs Bean 注册」的分界线。
...
量级分档：10 万级条件装配开销可忽略；千万级大量 starter 的条件评估拖慢启动（用 --debug 分析）；亿级需要精确控制装配面——量级决定装配的精确度要求。
