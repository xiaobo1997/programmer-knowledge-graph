...
从系统架构上下游看：AOP 代理是 Spring 容器与业务代码的中间层——容器在上游创建代理，业务在下游消费代理，模块边界即「代理 vs 目标」的分界线。
...
## 代理生命周期状态图：从 Bean 到代理

```mermaid
stateDiagram-v2
    [*] --> 原始Bean: 实例化完成
    原始Bean --> 代理判定: postProcessAfterInitialization
    代理判定 --> 创建代理: 有匹配的Advisor
    代理判定 --> 返回原始: 无匹配
    创建代理 --> [*]: 容器中注册的是代理
    返回原始 --> [*]
```

10 万 QPS 以内的切面场景，AOP 代理的性能开销可忽略；百万到千万级场景下代理链长与切点匹配频率成为可测量的瓶颈；亿级流量场景中 AOP 的架构定位需要重新评估（部分切面逻辑下沉到网关或 SDK 层）。
...
质疑：代理对象注入后，原始对象去哪了？追问一层：容器里同时存原始对象和代理对象吗？再深一层：如果切面移除，容器里是回滚到原始对象还是代理永生？
